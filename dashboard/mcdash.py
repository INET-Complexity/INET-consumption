...
def load_monte_carlo_results(results_dir):
    ...
    files = sorted(list(results_dir.glob("UK_run_*.csv")) + list(results_dir.glob("GBR_shallow_output_*.csv")))
    all_runs = []
    for file in files:
        try:
...
def load_var_forecast(results_dir, historical_data=None):
    ...
    try:
        ...
        if not gbr_cols:
            return None
        var_mapping = {"GBR_GDP": "Gross Output", "GBR_CPI": "CPI", "GBR_UNEMP": "Unemployment Rate"}
        var_forecast = pd.DataFrame()
        for var_col, std_col in var_mapping.items():
...
def main():
    ...
    with st.sidebar:
        ...
        alt_forecast = st.selectbox("Show alternative forecasts?", ["None", "VAR", "Treasury Average"])
        st.markdown(
            """
            <div style='height: 2rem;'></div>
        """,
