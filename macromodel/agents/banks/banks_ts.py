...
def create_banks_timeseries(bank_data: pd.DataFrame, scale: int) -> TimeSeries:
    ...
    return TimeSeries(
        ...
        dividend_fund_declared_distribution=np.zeros(len(bank_data)),
        dividend_fund_retained_capacity=np.zeros(len(bank_data)),
        dividend_fund_settlement_debit=np.zeros(len(bank_data)),
