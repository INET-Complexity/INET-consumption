...
class FirmTimeSeries(TimeSeries):
    ...
    def from_data(
        ...
        return cls(
            ...
            pricing_mc_smooth=np.where(data["Unit Costs"].values > 0.0, data["Unit Costs"].values, np.nan),
            pricing_ac=np.full(data.shape[0], np.nan),
            pricing_ac_smooth=np.where(data["Unit Costs"].values > 0.0, data["Unit Costs"].values, np.nan),
