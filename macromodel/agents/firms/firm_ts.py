...
class FirmTimeSeries(TimeSeries):
    ...
    def from_data(
        ...
        return cls(
            ...
            dividend_fund_cash_distributable_profit_candidate=np.zeros(data.shape[0]),
            dividend_fund_distress_gate_passed=np.zeros(data.shape[0], dtype=bool),
            dividend_fund_declared_distribution=np.zeros(data.shape[0]),
            dividend_fund_retained_capacity=np.zeros(data.shape[0]),
            dividend_fund_settlement_debit=np.zeros(data.shape[0]),
            ...
            target_operating_refinance_credit=np.zeros(data.shape[0]),
            total_target_operating_refinance_credit=[0.0],
            ...
            received_operating_refinance_credit=np.zeros(data.shape[0]),
            total_received_operating_refinance_credit=[0.0],
            ...
            operating_revolving_interest_base=np.zeros(data.shape[0]),
            operating_revolving_rate=np.zeros(data.shape[0]),
            operating_revolving_interest_paid=np.zeros(data.shape[0]),
            operating_revolving_opening_balance=np.zeros(data.shape[0]),
            operating_revolving_current_draw=np.zeros(data.shape[0]),
            operating_revolving_repayment=np.zeros(data.shape[0]),
            operating_revolving_closing_balance=np.zeros(data.shape[0]),
            ...
            total_credit_exposure=[(data["Debt"].values + np.maximum(0.0, -data["Deposits"].values)).sum()],
