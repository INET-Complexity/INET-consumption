...
``L^d_it = target_consumption_it + cash_rent_it + scheduled_debt_service_it - income_it``,

a positive value whenever desired consumption plus actual rent plus scheduled
debt service exceeds current income. Since GH #120, ``target_consumption``
carries market expenditure only, so actual (cash) rent enters as its own use;
imputed rent is excluded by construction because it is not a liability.

This module performs no I/O and does not touch any
...
def compute_liquidity_shortfall(
    ...
    cash_rent: np.ndarray | None = None,
    ...
    Args:
        income (np.ndarray): Current household income, ``y_it``. Gross of rent
            paid (see ``Households.compute_income``), which is why ``cash_rent``
            below is a separate use rather than already netted out here.
        target_consumption (np.ndarray): Desired consumption expenditure for
            the period. Since GH #120 this is the *market-expenditure* part of
            the CACF target: the housing-flow components have been carved out
            by ``CreditAugmentedConsumption`` before demand reaches firms.
        ...
        cash_rent (np.ndarray | None): Actual rent paid for the period, a real
            cash outflow, non-zero for renters and social-housing tenants only.
            Added to the period's cash uses because ``target_consumption`` no
            longer carries it. Imputed rent is deliberately *not* accepted
            here: it is an accounting component of measured consumption, never
            a liability, and must never trigger feasibility support. Defaults
            to zero so existing callers keep their previous semantics.

    Returns:
        LiquidityShortfallResult: ``liquidity_shortfall`` is ``L^d_it``,
            positive when desired consumption plus cash rent plus scheduled
            debt service exceeds income, zero or negative (a surplus)
            otherwise; ``0.0`` for any household-period with a non-finite
            input or negative ``scheduled_debt_service``. ``household_saving``
            is ``s_it = income - target_consumption - cash_rent``, recorded
            separately since it is the paper's primitive quantity and useful
            to validate the shortfall's sign independently; subject to the
            same fallback.
    """
    ...
    cash_rent = np.zeros_like(target_consumption) if cash_rent is None else np.asarray(cash_rent, dtype=float)

    valid = (
        np.isfinite(income)
        & np.isfinite(target_consumption)
        & np.isfinite(scheduled_debt_service)
        & np.isfinite(cash_rent)
        & (scheduled_debt_service >= 0.0)
    )

    household_saving = np.where(valid, income - target_consumption - cash_rent, 0.0)
    liquidity_shortfall = np.where(valid, target_consumption + cash_rent + scheduled_debt_service - income, 0.0)
