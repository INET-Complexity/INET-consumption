...
st.markdown(
    "This dashboard visualises the components of GDP when calculated using the three different methodologies. Each component listed in the legend has a + or - sign showing whether it should be added or subtracted from the total."
)
...
def load_data():
    ...
    if not os.path.exists(output_path):
        ...
        st.error(
            "Please ensure you are running this script from a directory containing an 'output' folder with 'GBR_gdp_components.csv'"
        )
        st.stop()
...
def create_gdp_chart(df, components, title, gdp_column):
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df[gdp_column], name="Total GDP", line=dict(color="black", width=2)))
...
with tab2:
    ...
    fig_expenditure = create_gdp_chart(
        df, expenditure_components, "GDP Expenditure Approach Components", "GDP_Expenditure"
    )
