...
# Set page config
st.set_page_config(page_title="Macromodel Validation", layout="wide", initial_sidebar_state="expanded")

# Constants
VARIABLES_OF_INTEREST = {"Gross Output": "Gross Output", "CPI": "CPI", "Unemployment Rate": "Unemployment Rate"}
CONFIDENCE_LEVELS = {"90%": 0.90, "10%": 0.10}
...
def find_data_pickle_candidates():
    candidates = []
    for candidate in [
        Path("data/processed_data/data.pkl"),
        Path("data/output_data/data.pkl"),
        Path("run_model/data/output_data/data.pkl"),
    ]:
        if candidate.exists():
            candidates.append(candidate)
    candidates.extend(path for path in Path.cwd().glob("**/data.pkl") if path not in candidates)
    return candidates


def _country_code_from_key(country_key):
    return str(getattr(country_key, "value", country_key))


def _get_synthetic_country(data, country_code):
    for key, country_data in data.synthetic_countries.items():
        if _country_code_from_key(key) == country_code:
            return country_data
    available = ", ".join(sorted(_country_code_from_key(key) for key in data.synthetic_countries))
    raise KeyError(f"Country {country_code} not found in data pickle. Available countries: {available}")


@st.cache_data
def list_data_country_codes(data_pkl_path):
    data = DataWrapper.init_from_pickle(str(data_pkl_path))
    return sorted(_country_code_from_key(key) for key in data.synthetic_countries)


def default_results_dir(country_code):
    candidates = [
        Path("run_model/data/output_data"),
        Path("data/output_data"),
        Path(f"output-MonteCarlo-{country_code}/monte_carlo"),
    ]
...
def find_results_dir_candidates(country_code):
    patterns = [
        f"monte_carlo_{country_code}.pkl",
        f"{country_code}_shallow_output*.csv",
        f"{country_code}_run_*.csv",
        "*shallow_output*.csv",
    ]
    candidates = []
    ...
    default_dir = Path(default_results_dir(country_code))
    if default_dir.exists() and default_dir not in candidates:
        candidates.insert(0, default_dir)
    return candidates
...
@st.cache_data
def load_historical_data(data_pkl_path, country_code):
    ...
    df = pd.DataFrame(
        {
            ...
            "Unemployment Rate": exo.labour_stats["Unemployment Rate (Value)"]
...
@st.cache_data
def load_monte_carlo_results(results_dir, country_code):
    """Load and process Monte Carlo simulation results, and use nominal Gross Output levels.
    Discovers country-specific run CSVs and shallow output CSVs."""
    results_dir = Path(results_dir).resolve()
    mc_pickle = results_dir / f"monte_carlo_{country_code}.pkl"
    if mc_pickle.exists():
        run_data = pd.read_pickle(mc_pickle)
        run_data = standardize_result_columns(run_data)
        if "CPI" in run_data.columns:
            if isinstance(run_data.index, pd.MultiIndex):
                run_data["CPI"] = run_data.groupby(level=0)["CPI"].transform(lambda x: x / x.iloc[0] * 100)
            else:
                cpi_base = run_data["CPI"].iloc[0]
                run_data["CPI"] = run_data["CPI"] / cpi_base * 100
        if isinstance(run_data.index, pd.MultiIndex):
            index_names = list(run_data.index.names)
            if index_names[0] is None or index_names[1] is None:
                run_data.index = run_data.index.set_names(["run", "time"])
            else:
                run_data.index = run_data.index.set_names(["run", "time"])
            return run_data
        return pd.concat([run_data], keys=[0], names=["run", "time"])
    ...
    if country_code == "GBR":
        patterns.extend(["UK_run_*.csv", "GBR_shallow_output_*.csv"])
    files = []
    for pattern in patterns:
        files.extend(path for path in results_dir.glob(pattern) if path not in files)
    if not files:
        files = sorted(path for path in results_dir.glob("*shallow_output*.csv") if "var_output" not in path.name)
    files = sorted(files)
    all_runs = []
    for file in files:
        try:
            ...
            continue
    if not all_runs:
        raise ValueError(f"No simulation results found for {country_code} in {results_dir}")
    combined_runs = pd.concat(all_runs, keys=range(len(all_runs)), names=["run", "time"])
    return combined_runs


@st.cache_data
def load_var_forecast(results_dir, historical_data=None, country_code="FRA"):
    """Load VAR forecast results, supporting both wide and long formats."""
    try:
        ...
        if set(["country", "variable", "period", "value"]).issubset(var_data.columns):
            country_data = var_data[var_data["country"] == country_code]
            # Aggregate by period and variable (mean)
            country_agg = country_data.groupby(["period", "variable"], as_index=False)["value"].mean()
            # Pivot so each variable is a column, indexed by period
            pivot = country_agg.pivot(index="period", columns="variable", values="value")
            ...
            if all(isinstance(p, (int, np.integer)) for p in period_values) or np.issubdtype(
                var_forecast.index.dtype, np.integer
            ):
        ...
        country_cols = [col for col in var_data.columns if country_code in col]
        if not country_cols:
            return None
        var_mapping = {
            f"{country_code}_GDP": "Gross Output",
            f"{country_code}_CPI": "CPI",
            f"{country_code}_UNEMP": "Unemployment Rate",
        }
        var_forecast = pd.DataFrame()
        for var_col, std_col in var_mapping.items():
...
def load_treasury_forecast(historical_data=None):
    ...
    try:
        ...
        treasury = pd.DataFrame({"Gross Output": [gdp_2014, gdp_2015], "CPI": [cpi_2014, cpi_2015]}, index=idx)
...
def create_plot(
    ...
    compare_label="Shock",
    shock_start_q=0,
    show_shock_annotation=False,
):
    """Create a single plot for a variable. If compare_stats/compare_run are set, add second set with distinct colors."""
    fig = go.Figure()
    sim_start = pd.Timestamp("2014-01-01")
    forecast_dates = pd.date_range(start=sim_start, periods=len(mc_stats[var_key]), freq="QE")
    fig.add_vrect(
        x0=forecast_dates[0], x1=forecast_dates[-1], fillcolor="rgba(200,255,200,0.2)", layer="below", line_width=0
    )
    if show_shock_annotation:
        shock_idx = max(0, min(shock_start_q, len(forecast_dates) - 1))
        fig.add_vrect(
            x0=forecast_dates[shock_idx],
            x1=forecast_dates[-1],
            fillcolor="rgba(255,210,100,0.2)",
            layer="below",
            line_width=0,
        )
        fig.add_annotation(
    ...
    fig.add_trace(
        go.Scatter(
            ...
            name=f"{p} 90% CI Upper".strip() or "90% CI Upper",
        )
    )
    fig.add_trace(
        go.Scatter(
            ...
            mode="lines",
            line_color="rgba(0,100,80,0.2)",
            name=f"{p} 90% CI Lower".strip() or "90% CI Lower",
        )
    )
    fig.add_trace(
        go.Scatter(
            ...
            name=f"{p} 75% CI Upper".strip() or "75% CI Upper",
        )
    )
    fig.add_trace(
        go.Scatter(
            ...
            name=f"{p} 75% CI Lower".strip() or "75% CI Lower",
        )
    )
    fig.add_trace(
        go.Scatter(
    ...
    if compare_stats is not None and compare_run is not None and var_key in compare_stats:
        ...
        fig.add_trace(
            go.Scatter(
                x=compare_dates,
                y=compare_stats[var_key]["q90"],
                fill=None,
                mode="lines",
                line_color="rgba(180,80,0,0.35)",
                name=f"{compare_label} 90% CI Upper",
            )
        )
        fig.add_trace(
            go.Scatter(
                x=compare_dates,
                y=compare_stats[var_key]["q10"],
                fill="tonexty",
                mode="lines",
                line_color="rgba(180,80,0,0.35)",
                name=f"{compare_label} 90% CI Lower",
            )
        )
        fig.add_trace(
            go.Scatter(
                x=compare_dates,
                y=compare_stats[var_key]["q75"],
                fill=None,
                mode="lines",
                line_color="rgba(220,140,0,0.15)",
                name=f"{compare_label} 75% CI Upper",
            )
        )
        fig.add_trace(
            go.Scatter(
                x=compare_dates,
                y=compare_stats[var_key]["q25"],
                fill="tonexty",
                fillcolor="rgba(255,255,255,0.2)",
                mode="lines",
                line_color="rgba(220,140,0,0.15)",
                name=f"{compare_label} 75% CI Lower",
            )
        )
        compare_vals = (
            compare_run[var_name].values[:n_compare]
            if len(compare_run[var_name]) >= n_compare
            else compare_run[var_name].values
        )
        fig.add_trace(
            go.Scatter(
                x=compare_dates[: len(compare_vals)],
                y=compare_vals,
    ...
    if show_var and var_forecast is not None and var_name in var_forecast.columns:
        ...
        fig.add_trace(
            go.Scatter(
                x=var_dates, y=var_forecast[var_name], name=label, line=dict(color="green", dash="dash", width=2)
            )
        )
    fig.add_trace(
        go.Scatter(x=historical_data.index, y=historical_data[var_name], name="Historical", line=dict(color="black"))
    )
    fig.update_layout(
        xaxis_title="Time", yaxis_title=var_key, showlegend=True, hovermode="x unified", xaxis=dict(tickformat="%Y-Q%q")
    )
    return fig
...
def compute_rmse_table(
    ...
    show_var=False,
    alt_label=None,
    var_forecast_var=None,
    var_forecast_treasury=None,
    compare_stats=None,
    compare_run=None,
    primary_label="Baseline",
    compare_label="Shock",
...
def create_single_var_comparison_chart(run_a, run_b, label_a, label_b, var_name):
    ...
    dates = pd.date_range(start=pd.Timestamp("2014-01-01"), periods=n, freq="QE")
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=dates, y=run_a[var_name].values[:n], name=label_a, line=dict(color="red")))
    fig.add_trace(go.Scatter(x=dates, y=run_b[var_name].values[:n], name=label_b, line=dict(color="blue", dash="dash")))
...
def main():
    ...
    with st.sidebar:
        ...
        data_pickle_candidates = find_data_pickle_candidates()
        if data_pickle_candidates:
            selected_data_pickle = st.selectbox(
                "Historical data pickle",
                options=data_pickle_candidates,
                format_func=lambda path: str(path),
            )
            data_pkl_path = str(selected_data_pickle)
            available_country_codes = list_data_country_codes(data_pkl_path)
        else:
            data_pkl_path = st.text_input(
                "Historical data pickle",
                value="data/processed_data/data.pkl",
                help="Path to the DataWrapper pickle used for historical validation data.",
            )
            available_country_codes = ["FRA"]

        default_country_index = available_country_codes.index("FRA") if "FRA" in available_country_codes else 0
        selected_country_code = st.selectbox(
            "Country",
            options=available_country_codes,
            index=default_country_index,
            help="Country code used for historical data, run CSV discovery, and VAR forecast filtering.",
        )

        path_type = st.selectbox("Select path type:", ["Median Path", "Mean Path", "Single Run"])

        if path_type == "Single Run":
            path_variable = st.selectbox("Variable for path selection:", ["Gross Output", "CPI", "Unemployment Rate"])
        ...
        alternative_forecast_options = ["None", "VAR"]
        if selected_country_code == "GBR":
            alternative_forecast_options.append("Treasury Average")
        alt_forecast = st.selectbox("Show alternative forecasts?", alternative_forecast_options)
        st.markdown(
            """
            <div style='height: 2rem;'></div>
        """,
        ...
        results_dir_candidates = find_results_dir_candidates(selected_country_code)
        if results_dir_candidates:
            selected_results_dir = st.selectbox(
                "Results Directory",
                options=results_dir_candidates,
                format_func=lambda path: str(path),
                help=(
                    f"Primary set of {selected_country_code} runs. Supports "
                    f"monte_carlo_{selected_country_code}.pkl, {selected_country_code}_run_*.csv, "
                    f"and {selected_country_code}_shallow_output*.csv."
                ),
            )
            results_dir = str(selected_results_dir)
        else:
            results_dir = st.text_input(
                "Results Directory",
                value=default_results_dir(selected_country_code),
                help=(
                    f"Primary set of {selected_country_code} runs. Supports "
                    f"monte_carlo_{selected_country_code}.pkl, {selected_country_code}_run_*.csv, "
                    f"and {selected_country_code}_shallow_output*.csv."
        ...
        if compare_mode:
            ...
            compare_dir = st.text_input(
                "Comparison results directory", value="output_baseline", help="Second set of runs to compare."
            )
            compare_label = st.text_input("Label for comparison set", value="Shock")
    try:
        full_hist_data = load_historical_data(data_pkl_path, selected_country_code)
        sim_start = pd.Timestamp("2014-01-01")
        ...
        mc_results = load_monte_carlo_results(results_dir, selected_country_code)
        ...
        var_forecast_df = load_var_forecast(
            results_dir,
        ...
        if compare_mode and compare_dir:
            mc_results_2 = load_monte_carlo_results(compare_dir, selected_country_code)
        ...
        for i, (var_key, var_name) in enumerate(VARIABLES_OF_INTEREST.items()):
            with tabs[i]:
                fig = create_plot(
                    ...
                    alt_label=alt_label,
                    compare_stats=mc_stats_2,
                    compare_run=selected_run_2,
                    primary_label=primary_label,
                    compare_label=compare_label,
                    shock_start_q=shock_start_q,
                    show_shock_annotation=show_shock_annotation,
                )
                ...
                rmse_table = compute_rmse_table(
                    ...
                    show_var=(alt_forecast != "None"),
                    alt_label=alt_label,
                    var_forecast_var=var_forecast_df,
                    var_forecast_treasury=treasury_forecast_df,
                    compare_stats=mc_stats_2,
                    compare_run=selected_run_2,
                    primary_label=primary_label,
                    compare_label=compare_label,
                ...
                st.info(
                    """RMSE (Root Mean Squared Error) is a standard measure of model fit. It quantifies the average magnitude of the error between model forecasts and actual observed values, with lower values indicating better fit. All values are shown as a percentage of the actual value."""
                )
                st.info(
                    """The VAR forecast uses historic data from the years prior to the simulation run to forecast the same period as the INET Oxford macromodel. Our VAR implementation follows Poledna et al 2023."""
                )
                if selected_country_code == "GBR":
                    st.info(
                        """The Treasury average forecast is the mean of a range of forecasters made by private sector forecasters in the first period of our simulation window. See the official [Treasury forecast database](https://www.gov.uk/government/statistics/database-of-forecasts-for-the-uk-economy) for more information."""
                    )
                ...
                if compare_mode and selected_run_2 is not None:
                    ...
                    comp_table = build_selected_run_pct_change_table(selected_run, selected_run_2, var_name, pct_label)
