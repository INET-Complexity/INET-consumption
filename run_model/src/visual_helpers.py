...
def plot_output(df, no_rows, no_cols, country_code, x_col=None):
    ...
    for idx, col_name in enumerate(columns_to_plot):
        ...
        fig.add_trace(go.Scatter(x=x, y=df[col_name], mode="lines", name=col_name, showlegend=False), row=row, col=col)

    fig.update_layout(height=200 * no_rows, width=300 * no_cols, title_text=country_code, template="plotly_white")
    fig.show()
