...
def test__mapping_rebases_log_income_and_bootstraps_lags_from_design_matrix_during_warmup():
    ...
    x_t = build_permanent_income_forecast_regressors(
        sources=PermanentIncomeSimulationSources(
            ...
            real_pc_income=[10.0, 11.0, 12.0, 13.0],
    ...
    initial_design_log_y = np.log(200.0 + 4.0)
    assert x_t["log_real_pc_income"] == pytest.approx(np.log(np.exp(initial_design_log_y) * 13.0 / 10.0))
...
def test__mapping_uses_start_period_as_income_index_anchor_when_history_contains_it():
    ...
    x_t = build_permanent_income_forecast_regressors(
        sources=PermanentIncomeSimulationSources(
            ...
            real_pc_income=[10.0, 10.5, 11.0, 12.0, 15.0],
            policy_rate=[0.01] * 5,
    ...
    assert x_t["log_real_pc_income"] == pytest.approx(np.log(200.0 * 15.0 / 10.0))


def test__mapping_uses_endogenous_income_real_rate_and_unemployment_after_warmup():
    periods = pd.period_range("2014Q1", periods=20, freq="Q")
    design = _design_matrix(periods)
    real_pc_income = np.linspace(10.0, 18.0, 17)
    policy_rate = np.linspace(0.005, 0.025, 17)
    cpi = np.linspace(100.0, 120.0, 17)
    unemployment_decimal = np.linspace(0.05, 0.10, 17)
    ...
    x_t = build_permanent_income_forecast_regressors(
        sources=PermanentIncomeSimulationSources(
            ...
            unemployment_rate=unemployment_decimal,
        ),
        design_matrix=design,
        start_period="1980Q1",
    )
    ...
    assert x_t["unemp_rate_ma4_l1"] == pytest.approx((100.0 * unemployment_decimal[-5:-1]).mean())
    assert x_t["unemp_rate_ma4_l5"] == pytest.approx((100.0 * unemployment_decimal[-9:-5]).mean())
