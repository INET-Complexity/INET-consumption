...
def _settle(**overrides):
    values = {
        ...
        "base_ifa": np.asarray([50.0, 3.0, 0.0]),
...
def test__settles_liquidation_into_lfa_and_ifa_once():
    ...
    np.testing.assert_allclose(result.post_liquidation_lfa, [108.0, 23.0, 5.0])
    np.testing.assert_allclose(result.post_liquidation_ifa, [42.0, 0.0, 0.0])
    np.testing.assert_allclose(result.settled_liquidation_total, [8.0, 3.0, 0.0])
    np.testing.assert_allclose(result.residual_shortfall_after_settlement, [0.0, 6.0, 0.0])
...
def test__zero_liquidation_preserves_financial_bases():
    ...
    np.testing.assert_allclose(result.post_liquidation_ifa, [50.0, 3.0, 0.0])
    np.testing.assert_allclose(result.residual_shortfall_after_settlement, [0.0, 4.0, 0.0])


def test__unexecuted_liquidation_is_restored_to_residual_shortfall():
    result = _settle(
    ...
    np.testing.assert_allclose(result.settled_liquidation_total, [2.0, 3.0, 0.0])
    np.testing.assert_allclose(result.residual_shortfall_after_settlement, [7.0, 6.0, 0.0])
