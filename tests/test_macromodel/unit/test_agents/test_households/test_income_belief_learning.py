...
def test__income_belief_learning_outputs_use_correct_variance_mapping_and_prediction_error():
    ...
    expected_signal = np.log(current_income) - np.log(lagged_income)
    ...
    np.testing.assert_allclose(outputs.permanent_income_log_ratio, 0.2 + outputs.posterior_mean)
...
def test__income_belief_learning_accepts_updated_prior_state():
    priors = {
        ...
        "income_belief_rho": np.array([1.0]),
    ...
    expected_predicted_mean = 0.4
    expected_predicted_variance = 1.25
